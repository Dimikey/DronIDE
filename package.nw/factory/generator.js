...
Bdf.Generator.updateGenerator = function(block) {
  ...
  code.push("  // #include - ниже следует добавить бибилотеки, необходимые для модуля");
  code.push("  // Blockly.Arduino.definitions_['уникальное имя'] = '#include <библиотека>';\n");
  
  code.push("  // #define - если требуется - указать дефайны");
  code.push("  // Blockly.Arduino.definitions_['уникальное имя'] = '#define что-то чем-то';\n");
  
  code.push("  // void setup() - тут описывается код, который будет присутсвовать в секции SETUP");
  code.push("  // Blockly.Arduino.setups_['уникальное имя'] = текст, который должен быть в блоке SETUP\n");
  
  code.push("  // void loop() собственно то, что в code и будет в блоке loop");
  code.push("  //Blockly.Arduino.loops_['уникальное имя'] = ТО, что выполняется в LOOP но не поместу установки блока (игнорирует логику и структуру программы)");

  code.push("  var code = \'то, что ставится по месту установки блока согласно логике программы\';");
