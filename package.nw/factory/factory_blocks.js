...
Blockly.Blocks['field_ardutech_port'] = {
  // Set a dropdown field to selected an Arduino pin
  init: function() {
    this.setColour(160);
    this.appendDummyInput()
        .appendField('Ardutech port')
        .appendField(new Blockly.FieldDropdown(ports), "PORT_TYPE")
        .appendField('dropdown,')
        .appendField(new Blockly.FieldTextInput('NAME'), 'FIELDNAME');
    this.setInputsInline(true);
    this.setPreviousStatement(true, 'Field');
    this.setNextStatement(true, 'Field');
    this.setTooltip('Add a dropdown to select an Ardutech port.');
    this.setHelpUrl('');
  },
  onchange: function() {
    if (!this.workspace) {
