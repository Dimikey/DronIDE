 ...
 * to download.
 */
var projectName = 'ArdutechPRJ';
Bdf.saveXmlFactory = function() {
    var xmlData = Bdf.Factory.getXmlString();
    var blob = new Blob([xmlData], {type: 'text/xml'});
    var block = new Blob([document.getElementById('languagePre').textContent], {type:'text/plain'});
    var code = new Blob([document.getElementById('generatorPre').textContent], {type:'text/plain'});
    
    projectName = document.getElementById('languagePre').textContent.split('\n')[0]
                                            .replace("Blockly.Blocks['", "")
                                            .replace("'] = {", "");
    
    alert("Сохранение проекта: " + projectName)  
    saveAs(blob, projectName + '.bfc');
    alert("Сохранение файла описания модуля")  
	saveAs(block, projectName + '_block.js');
	alert("Сохранение файла поведения модуля")  
    saveAs(code, projectName + '_code.js');
